...
\paragraph{Firmware:}
The firmware must be encrypted and signed on the firmware management server and later decrypted by the host devices that process and execute the update.
...
The host uses the manifest to execute the necessary steps to flash the device with the new firmware image.
...
\begin{table}[h!]
  ...
  \hline
   ...
   \hline
   Key encryption key & Encrypted, Signed \\
  \hline
  \end{tabular}
