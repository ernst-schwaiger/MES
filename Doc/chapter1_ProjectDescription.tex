...
\begin{itemize}
    \item The key for signing the package on the build system should directly be created on the token to prevent it being copied around.
    \item Do we require a server authentication using ssh/scp?
    \item Shall DTLS/CoAPS/OSCORE be used for securing communication between Clients and FW Manager? Is encrypting the payload sufficient?
\end{itemize}
