\section{Key Management}\label{sec:keyManagement}
...
\begin{itemize}
    \item Type: Asymmetric key pair for a Certification Authority, RSA.
    ...
    \begin{itemize}
        ...
        \item Signs the Client\_Encryption certificates that bind each client public key to a client identity.
    \end{itemize}
    \item Properties:
    \begin{itemize}
        \item Self-signed and never used directly for firmware signatures or encryption.
        \item Stored offline and only used to issue or revoke subordinate certificates.
        \item Acts as the single trust anchor for certificate validation in the system.
    \end{itemize}
\end{itemize}
...
\subsubsection{
Client node encryption key pair and certificate (Client\_Encryption)
}\label{subsubsec:clientNodeKeyPairs}

\begin{itemize}
    \item Type: Asymmetric key pair for Elliptic Curve Diffie-Hellman based on Curve25519 (X25519) supported by the RIOT-OS cryptographic library C25519. The key pair is denoted $(d_R, Q_R)$ in Section \ref{sec:dataEncryption}.
    \item Owner: Individual Client Node.
    \item Role:
    \begin{itemize}
        \item Acts as the long term receiver key pair for the ECIES scheme used to protect the Advanced Encryption Standard session key $K_{\text{enc}}$ that encrypts the firmware image.
        \item Provides the receiver public key $Q_R$ that the Firmware Management Server uses for ECIES encryption of $K_{\text{enc}}$.
    \end{itemize}
    \item Properties:
    \begin{itemize}
        \item The private key $d_R$ remains on the Client Node and is never sent over the network.
        \item The public key $Q_R$ is bound to the client identity by an X.509 certificate (Client\_Encryption) signed by the Root Certification Authority.
        \item The Firmware Management Server stores the Client\_Encryption certificate for each client and extracts $Q_R$ from that stored certificate when performing ECIES.
        \item The Client Node stores the private key $d_R$ and, optionally, its own Client\_Encryption certificate for identification and consistency checks.
    \end{itemize}
\end{itemize}
...
\subsubsection{
Root Certification Authority and signing certificate keys
}\label{subsubsec:rootCAAndCertificateKeys}

\paragraph{} The Root Certification Authority and the two signing key pairs are generated once during system initialization.

\begin{enumerate}
    \item Generate a Root Certification Authority RSA key pair on a secure, offline machine.
    \item Create a self-signed Root Certification Authority certificate that identifies the authority and contains the Root Certification Authority public key.
    \item On the Build System, create an RSA key pair for the Build\_Signing certificate on a hardware crypto token. Export only the public key.
    ...
    \item On the Root Certification Authority machine, issue two certificates: one for the Build\_Signing key and one for the Mgmnt\_Signing key. Both certificates are signed with the Root Certification Authority key.
    \item Install the Root Certification Authority certificate and the Build\_Signing certificate on the Firmware Management Server.
    \item Install the Root Certification Authority certificate and the Mgmnt\_Signing certificate on all Client Nodes.
\end{enumerate}



\subsubsection{
Client node encryption key pairs and certificate issuance
}\label{subsubsec:clientNodeKeyPairsKeyGeneration}

\paragraph{} Client encryption keys are generated during provisioning of each device using a trusted provisioning channel. The Firmware Management Server stores a client certificate for each client in order to obtain the public key for ECIES encryption.

\begin{enumerate}
    \item A provisioning station (for example the Build System in a controlled environment) generates a Curve25519 compatible key pair $(d_R, Q_R)$ for the Client Node.
    \item The private key $d_R$ is installed onto the Client Node during provisioning and stored in non volatile memory. The private key is marked as secret and is never transmitted during normal operation.
    \item The public key $Q_R$, together with a device identifier, is transferred to the Firmware Management Server and used to request certificate issuance.
    \item The Root Certification Authority issues an X.509 Client\_Encryption certificate that contains $Q_R$ as subject public key and signs this certificate with the Root Certification Authority private key.
    \item The Firmware Management Server stores the issued Client\_Encryption certificate in a database entry that is associated with the identity of the Client Node.
    \item The issued Client\_Encryption certificate is optionally transferred to the Client Node over the trusted provisioning channel and stored on the Client Node for traceability and consistency checks.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item For each Client Node the Firmware Management Server loads the stored Client\_Encryption certificate for that client, extracts the public key $Q_R$ from the certificate, computes the shared secret $S = d_E \cdot Q_R$, derives $K_{\text{wrap}}$ from $S$ via HKDF, and encrypts $K_{\text{enc}}$ to obtain $C$, as described in Section \ref{sec:dataEncryption}.
...
\begin{itemize}
    ...
    \end{itemize}
    \item Root Certification Authority certificate and subordinate certificates:
    \begin{itemize}
    ...
    \end{itemize}
    \item Client certificates (Client\_Encryption):
    \begin{itemize}
        \item Stored in a database or configuration store that maps each Client Node identity to its Client\_Encryption certificate.
        \item The Firmware Management Server extracts $Q_R$ from the stored certificate when performing ECIES encryption of $K_{\text{enc}}$.
...
\begin{enumerate}
    \item The Root Certification Authority certificate is embedded into the Client Node firmware image or written into a dedicated read only configuration partition.
    \item The Mgmnt\_Signing certificate is installed on the Client Nodes as part of the initial firmware image.
    \item Each Client Node gets their encryption key pair $(d_R, Q_R)$ as described in Section \ref{subsubsec:clientNodeKeyPairs}.
    \item The public key $Q_R$, together with an identifier for the Client Node, is transmitted over a trusted provisioning channel to the Firmware Management Server.
    \item The Firmware Management Server issues a Client\_Encryption certificate containing $Q_R$, signed by the Root Certification Authority.
    \item The Firmware Management Server stores the Client\_Encryption certificate associated with the device identity in its client database.
    \item The issued Client\_Encryption certificate is transferred to the Client Node over the trusted provisioning channel and stored on the Client Node.
\end{enumerate}
...
\begin{enumerate}
    \item The Build System sends the firmware update package and the detached signature generated with the Build\_Signing key to the Firmware Management Server via Secure Shell.
    \item The Firmware Management Server verifies the signature using the Build\_Signing certificate that is already installed on the Firmware Management Server and validates that certificate against the Root Certification Authority certificate.
    \item After verification and after packaging of the firmware into a SUIT update, the Firmware Management Server encrypts the SUIT firmware with $K_{\text{enc}}$ and encrypts $K_{\text{enc}}$ for each Client Node as $(Q_E, C)$ using ECIES. For ECIES, the Firmware Management Server extracts each client public key $Q_R$ from the stored Client\_Encryption certificate.
